...
export class SalaryDetailsPage {
  ...
  async fillSalaryDetails(salary: string, effectiveFrom: string, reason: string, payrollNumber: string) {
    await this.page.waitForSelector('#effectiveFrom', { state: 'visible', timeout: 150000 });
    ...
    await this.page.locator('#hourlyRate').clear();
    await this.page.locator('#hourlyRate').pressSequentially('10');
    await this.page.locator('#hourlyRate').press('Tab');
    await this.page.waitForSelector('#effectiveFrom', { state: 'visible', timeout: 150000 });
    await this.page.locator('#effectiveFrom').click();
    await this.page.getByRole('textbox', { name: 'Effective from' }).fill(effectiveFrom);
