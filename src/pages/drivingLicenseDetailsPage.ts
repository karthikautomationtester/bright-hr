...
export class DrivingLicenseDetailsPage {
  ...
  async fillDrivingLicenseDetails(licenceNumber: string, drivingLicenceCountryOfIssue: string, licenceClass: string, dateOfExpiry: string) {
    await this.page.getByRole('textbox', { name: 'Licence number' }).pressSequentially(licenceNumber);
    ...
    await this.page.getByRole('textbox', { name: 'Licence class' }).pressSequentially(licenceClass);
    await this.page.getByRole('textbox', { name: 'Date of expiry' }).pressSequentially(dateOfExpiry);
  }
}